...
import { useStore } from "@/context/StoreContext"; // <--- USE THIS
import { FadeIn } from "@/components/ui/Motion";
import { Plus, Search, Edit2, Trash2 } from "lucide-react";
import { useState } from "react";

export default function InventoryPage() {
  const { products, deleteProduct } = useStore(); // <--- Get dynamic data
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Search Input and Add Button... same code ... */}
            ...
            {filteredProducts.map((product) => (
              <tr key={product.id} className="hover:bg-white/5 transition-colors group">
                <td className="p-6">
                  ...
                  <div className="text-xs text-[#C9D1D9] opacity-70">{product.variants?.length || 0} Variants</div>
                </td>
                <td className="p-6">
                ...
                <td className="p-6 text-right">
                  <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {/* UPDATED: DELETE BUTTON WORKS NOW */}
                    <button 
                      onClick={() => {
